...
if "cpf_logado" not in st.session_state:
    ...
    cpf = st.selectbox("Selecione seu nome", options=list(usuarios.values()))
    cpf_invertido = {v: k for k, v in usuarios.items()}
    if st.button("Entrar"):
        st.session_state.cpf_logado = cpf_invertido[cpf]
        st.session_state.entregador_nome = cpf
